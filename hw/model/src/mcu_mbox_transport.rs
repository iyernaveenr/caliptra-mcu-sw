// Use HW model mcu manager mbox transport for MCU mailbox tests.
// This serves as a common transport layer for tests involving MCU mailbox communication as the requester mode.

use registers_generated::mci;
use registers_generated::mci::bits::{MboxCmdStatus, MboxExecute, MboxLock, Notif0IntrTrigT};
...
impl McuMailboxTransport {
    ...
    pub fn execute(&self, cmd: u32, payload: &[u8]) -> Result<(), McuMailboxError> {
        ...
        // Wait for the mailbox lock to be released
        let mut retry = 1000;
        while self.mci.mcu_mbox0_csr_mbox_lock.get() != 0 {
            if retry == 0 {
                println!("[xs debug] Timeout waiting for MCU mailbox lock");
        ...
        let len_words = payload.len() / size_of::<u32>();
        let word_bytes = &payload[..len_words * size_of::<u32>()];
