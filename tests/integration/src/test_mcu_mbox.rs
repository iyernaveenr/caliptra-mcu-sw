...
pub mod test {
    ...
    fn start_mcu_mbox_tests(feature: &str) {
        ...
        println!("[xs debug] MCI PTR: {:#X}", mci_ptr);
    ...
    pub fn test_mcu_mbox_soc_requester_loopback() {
        start_mcu_mbox_tests("test-mcu-mbox-soc-requester-loopback");
    }
    ...
    impl RequestResponseTest {
        ...
        fn prep_test_messages(&mut self, feature: &str) {
            if feature == "test-mcu-mbox-soc-requester-loopback" {
            ...
            } else if feature == "test-mcu-mbox-usermode" {
